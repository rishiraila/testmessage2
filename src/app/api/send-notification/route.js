...
function initFirebaseAdmin() {
  ...
  // Prefer env vars, but fall back to the hard-coded values above.
  ...
  const privateKey = (process.env.FIREBASE_PRIVATE_KEY || FALLBACK_PRIVATE_KEY).replace(/\\n/g, "\n");
...
/**
 * POST body shape supported:
 * - token: single device token (string)
 * - tokens: array of tokens (["token1","token2"])
 * - topic: topic string (e.g. "allUsers")
...
export async function POST(req) {
  try {
    ...
    return NextResponse.json({ success: false, error: "Firebase init failed", details: err?.message || String(err) }, { status: 500 });
  }
  ...
  try {
    ...
    // Build common message payload (notification + data)
    const notification = {
    ...
    const data = {
      ...
      // you can add more custom keys here
    };

    // If sending to a topic
    if (topic) {
      const message = {
        topic,
        notification,
    ...
    if (tokens && tokens.length) {
      const multicastMessage = {
        ...
        apns: {
          headers: { "apns-priority": "10" },
          payload: {
            aps: { alert: { title: notification.title, body: notification.body }, sound: "default", "mutable-content": 1 },
          },
        },
      };
