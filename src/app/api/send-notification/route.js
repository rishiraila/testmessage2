...
const FALLBACK_CLIENT_EMAIL =
  "firebase-adminsdk-fbsvc@testmessage-30914.iam.gserviceaccount.com";
...
function initFirebaseAdmin() {
  ...
  const clientEmail =
    process.env.FIREBASE_CLIENT_EMAIL || FALLBACK_CLIENT_EMAIL;
  const privateKeyRaw =
    process.env.FIREBASE_PRIVATE_KEY || FALLBACK_PRIVATE_KEY;
...
export async function POST(req) {
  ...
  try {
    ...
    if (!token && !(tokens && tokens.length) && !topic) {
      return NextResponse.json(
        { success: false, error: "Provide token OR tokens[] OR topic" },
        { status: 400 }
      );
    }
    ...
    const webpushPayload = {
      ...
      notification: {
        title: finalTitle,
        body: finalBody,
        image: finalImage, // correct place for web push image
      },
      fcm_options: {
        // optional: include a link to open when clicked
